...
<body class="page-header-fixed">
				...
				<div class="row-fluid">
					<div class="span12">
						<div class="portlet box light-grey">
							...
							<div class="portlet-title">
								<div class="caption"><i class="icon-globe"></i>首页板块列表</div>								
								<div class="btn-group pull-right">

									<button onclick="location='<%=path %>/indexPic/toIndexPicEdit'" class="btn green">
							...
							<div class="portlet-body">
								<table class="table table-striped table-bordered table-hover" id="userList">
									...
									<tbody>
										<c:forEach items="${indexPicList.beanList }" var="indexPicList" varStatus="status" >										
											<tr class="odd gradeX">
												<td>${status.count} </td>
												<td>${indexPicList.title}</td>
												<td>${indexPicList.secondTitle}</td>
												<td class="indexPicText">${indexPicList.text}</td>
												<c:if test="${indexPicList.isUse eq 0}">
													<td>启用</td>
												</c:if>
												<c:if test="${indexPicList.isUse eq 1}">
													<td>禁用</td>
												</c:if>	

												<td>
													<a href="<%=path%>/indexPic/toIndexPicDetail?id=${indexPicList.id}" class="btn mini purple  icn-only" title="查看"><i class="halflings-icon white user "></i></a>														
													<a href="<%=path %>/indexPic/toIndexPicEdit?id=${indexPicList.id}" class="btn mini blue icn-only" title="编辑"><i class="halflings-icon white pencil "></i></a>
													<c:if test="${indexPicList.isUse eq 0}">
														<a href="javascript:void(0);" onclick="forbiddenIndexPic('${indexPicList.id}')"  class="btn mini red icn-only" title="禁用"><i class="halflings-icon white ban-circle"></i></a>																																																												
													</c:if>
													<c:if test="${indexPicList.isUse eq 1}">
														<a href="javascript:void(0);" onclick="usingIndexPic('${indexPicList.id}')"  class="btn mini red icn-only" title="启用"><i class="halflings-icon white ok-circle"></i></a>																																																											
													</c:if>
												</td>
											</tr>
										</c:forEach>
								...
								<div class="pull-right">
									...
									<ul>	
										<%-- 如果总页数大于1页，那么《显示出来！ --%>									
										<c:if test="${indexPicList.pc > 1 }">						
											<li>
						            			  <a href="<%=path%>/indexPic/getIndexPicList?pc=${indexPicList.pc-1}" aria-label="Previous">
										...
										<c:choose>
											...
											<c:when test="${indexPicList.tp <= 10 }">
												<c:set var="begin" value="1" />
												<c:set var="end" value="${indexPicList.tp }" />
											</c:when>
											<c:otherwise>
												<%-- 当总页数>10时，通过公式计算出begin和end --%>
												<c:set var="begin" value="${indexPicList.pc-5 }" />
												<c:set var="end" value="${indexPicList.pc+4 }" />
												<%-- 头溢出 --%>
												<c:if test="${begin < 1 }">
													<c:set var="begin" value="1" />
												...
												<%-- 尾溢出 --%>
												<c:if test="${end > indexPicList.tp }">
													<c:set var="begin" value="${indexPicList.tp - 9 }" />
													<c:set var="end" value="${indexPicList.tp }" />
												</c:if>
											</c:otherwise>
										</c:choose>
										...
										<c:forEach var="i" begin="${begin }" end="${end }">
											<c:choose>
												<c:when test="${i eq indexPicList.pc }">
													 <li><a>${i }</a></li>
												</c:when>
												<c:otherwise>
												 <li><a href="<%=path%>/indexPic/getIndexPicList?pc=${i}">${i }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>        
							            <c:if test="${indexPicList.pc < indexPicList.tp }">															
											<li>
					             			 <a href="<%=path%>/indexPic/getIndexPicList?pc=${indexPicList.pc+1}" aria-label="Next">
					...
					<div class="span12">
						<div class="portlet box light-blue">
							...
							<div class="portlet-title">
								<div class="caption"><i class="icon-globe"></i>菜单板块列表</div>								
							</div>
							...
							<div class="portlet-body">
								<table class="table table-striped table-bordered table-hover" id="userList">
									...
									<thead>
										<tr>
											...
											<th width="45%">菜单描述</th>
											<th width="10%">是否启用</th>
											<th width="10%">是否首页展示</th>
											<th width="10%">操作</th>
										</tr>
									</thead>

									<tbody>
										<c:forEach items="${menuList.beanList }" var="menuList" varStatus="status" >										
											<tr class="odd gradeX">
												<td>${menuList.menuOrder} </td>
												<td>${menuList.name}</td>
												<td class="menuText">${menuList.text}</td>
												<c:if test="${menuList.isUse eq 0}">
													<td>启用</td>
												</c:if>
												<c:if test="${menuList.isUse eq 1}">
													<td>禁用</td>
												</c:if>	

												<c:if test="${menuList.isShow eq 0}">
													<td>展示</td>
												</c:if>
												<c:if test="${menuList.isShow eq 1}">
													<td>不展示</td>
												</c:if>	

												<td>
													<a href="<%=path%>/menu/toMenuDetail?id=${menuList.id}" class="btn mini purple  icn-only" title="查看"><i class="halflings-icon white user "></i></a>														
													<c:if test="${menuList.isShow eq 0}">
														<a href="javascript:void(0);" onclick="notShowMenu('${menuList.id}')"  class="btn mini red icn-only" title="不展示"><i class="halflings-icon white eye-close"></i></a>																																																												
													</c:if>
													<c:if test="${menuList.isShow eq 1}">
														<a href="javascript:void(0);" onclick="showMenu('${menuList.id}')"  class="btn mini red icn-only" title="展示"><i class="halflings-icon white eye-open"></i></a>																																																											
													</c:if>
												</td>
											</tr>
										</c:forEach>
								...
								<div class="pull-right">
									...
									<ul>	
										<%-- 如果总页数大于1页，那么《显示出来！ --%>									
										<c:if test="${menuList.pc > 1 }">						
											<li>
						            			  <a href="<%=path%>/menu/getMenuList?pc=${menuList.pc-1}" aria-label="Previous">
						            			    <span aria-hidden="true">&laquo;</span>
						              			  <span class="sr-only">Previous</span>
						             			 </a>
						           			 </li>
										...
										<c:choose>
											<%-- 如果总页数不足10页，那么把所有的页数都显示出来！ --%>
											<c:when test="${menuList.tp <= 10 }">
												<c:set var="begin" value="1" />
												<c:set var="end" value="${menuList.tp }" />
											</c:when>
											<c:otherwise>
												<%-- 当总页数>10时，通过公式计算出begin和end --%>
												<c:set var="begin" value="${menuList.pc-5 }" />
												<c:set var="end" value="${menuList.pc+4 }" />
												<%-- 头溢出 --%>
												<c:if test="${begin < 1 }">
													<c:set var="begin" value="1" />
													<c:set var="end" value="10" />
												...
												<%-- 尾溢出 --%>
												<c:if test="${end > menuList.tp }">
													<c:set var="begin" value="${menuList.tp - 9 }" />
													<c:set var="end" value="${menuList.tp }" />
												</c:if>
											</c:otherwise>
										</c:choose>
										...
										<c:forEach var="i" begin="${begin }" end="${end }">
											<c:choose>
												<c:when test="${i eq menuList.pc }">
													 <li><a>${i }</a></li>
												</c:when>
												<c:otherwise>
												 <li><a href="<%=path%>/menu/getMenuList?pc=${i}">${i }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>        
							            <c:if test="${menuList.pc < menuList.tp }">															
											<li>
					             			 <a href="<%=path%>/menu/getMenuList?pc=${menuList.pc+1}" aria-label="Next">
					              			  <span aria-hidden="true">&raquo;</span>
					              			  <span class="sr-only">Next</span>
					             			 </a>
					           				 </li>
	...
	<script src="<%=path%>/media/js/app.js" type="text/javascript"></script>
 	<script src="<%=path%>/media/js/manager/indexPic/indexPicList.js" type="text/javascript"></script>

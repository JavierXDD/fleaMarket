...
<body class="page-header-fixed">
					...
					<div class="span12">
						<div class="portlet box light-grey">
							...
							<div class="portlet-title">
								<div class="caption"><i class="icon-globe"></i>${pMneu.name}-二级菜单列表</div>
								<input type="hidden" name="parentsId" id="parentsId" value="${pMneu.id}"/>
								<div class="btn-group pull-right">
									<button onclick="addSonMenu()"
										class="btn green">
							...
							<div class="alert hidden" id="alert">
								...
								<strong>警告!</strong> 请逐条添加或修改!
							</div>
							<div class="portlet-body">
								<table class="table table-striped table-bordered table-hover" id="sonMenuList">

									<thead>
										<tr>
											<th width="10%">序号</th>
											<th width="15%">菜单名</th>
											<th width="45%">菜单url</th>
											<th width="10%">是否启用</th>
									...
									<tbody id="tbody">
										<c:forEach items="${sonMenuList.beanList }" var="sonMenuList" varStatus="status" >										
											<tr class="odd gradeX">
												<td>${status.count}</td>
												<td>${sonMenuList.name}</td>
												<td>${sonMenuList.url}</td>
												<c:if test="${sonMenuList.isUse eq 0}">
													<td>启用</td>
												</c:if>
												<c:if test="${sonMenuList.isUse eq 1}">
													<td>禁用</td>
												</c:if>	

												<td>
													<a href="javascript:void(0);" onclick="editSonMenu('${sonMenuList.id}',this)" class="btn mini purple  icn-only" title="编辑子菜单"><i class="halflings-icon white edit "></i></a>														
													<c:if test="${sonMenuList.isUse eq 0}">
														<a href="javascript:void(0);" onclick="forbiddenMenu('${sonMenuList.id}')" class="btn mini red  icn-only" title="禁用子菜单"><i class="halflings-icon white remove"></i></a>														
													</c:if>
													<c:if test="${sonMenuList.isUse eq 1}">
														<a href="javascript:void(0);" onclick="usingMenu('${sonMenuList.id}')" class="btn mini green  icn-only" title="启用子菜单"><i class="halflings-icon white ok"></i></a>	
													</c:if>													
												</td>
											</tr>
										</c:forEach>
								...
								<div class="pull-right">
									...
									<ul>	
										<%-- 如果总页数大于1页，那么《显示出来！ --%>									
										<c:if test="${sonMenuList.pc > 1 }">						
											<li>
						            			  <a href="<%=path%>/menu/getSonMenuList?pc=${sonMenuList.pc-1}" aria-label="Previous">
										...
										<c:choose>
											...
											<c:when test="${sonMenuList.tp <= 10 }">
												<c:set var="begin" value="1" />
												<c:set var="end" value="${sonMenuList.tp }" />
											</c:when>
											<c:otherwise>
												<%-- 当总页数>10时，通过公式计算出begin和end --%>
												<c:set var="begin" value="${sonMenuList.pc-5 }" />
												<c:set var="end" value="${sonMenuList.pc+4 }" />
												<%-- 头溢出 --%>
												<c:if test="${begin < 1 }">
													<c:set var="begin" value="1" />
												...
												<%-- 尾溢出 --%>
												<c:if test="${end > sonMenuList.tp }">
													<c:set var="begin" value="${sonMenuList.tp - 9 }" />
													<c:set var="end" value="${sonMenuList.tp }" />
												</c:if>
											</c:otherwise>
										</c:choose>
										...
										<c:forEach var="i" begin="${begin }" end="${end }">
											<c:choose>
												<c:when test="${i eq sonMenuList.pc }">
													 <li><a>${i }</a></li>
												</c:when>
												<c:otherwise>
												 <li><a href="<%=path%>/menu/getSonMenuList?pc=${i}">${i }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>        
							            <c:if test="${sonMenuList.pc < sonMenuList.tp }">															
											<li>
					             			 <a href="<%=path%>/menu/getSonMenuList?pc=${sonMenuList.pc+1}" aria-label="Next">
	...
	<script src="<%=path%>/media/js/app.js" type="text/javascript"></script>
 	<script src="<%=path%>/media/js/manager/menu/sonMenuList.js" type="text/javascript"></script>

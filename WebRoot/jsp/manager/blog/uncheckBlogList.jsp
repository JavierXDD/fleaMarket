...
<body class="page-header-fixed">
					...
					<div class="span12">
						<div class="portlet box light-grey">
							...
							<div class="portlet-title">						
								<div class="caption"><i class="icon-globe"></i>发帖列表</div>
							</div>
									...
									<tbody>
										<c:forEach items="${blogList.beanList }" var="blogList" varStatus="status" >										
											<tr class="odd gradeX">
												...
												<td>${blogList.name}</td>
												<td>${blogList.typeName}</td>
												<c:if test="${blogList.isUse eq 0}">
													<td><span class="label label-warning">未审核</span></td>
												</c:if>
												<c:if test="${blogList.isUse eq 1}">
													<td><span class="label label-success">审核通过</span></td>
												</c:if>
												<c:if test="${blogList.isUse eq 2}">
													<td><span class="label label-danger">审核未通过</span></td>
												</c:if>																								
												

												<td>
													<a href="<%=path%>/blog/toMyBlogDetail?id=${blogList.id}" class="btn mini purple icn-only" title="查看"><i class="halflings-icon white user "></i></a>													
													<a href="<%=path %>/blog/closeBlog?id=${blogList.id}" class="btn mini red icn-only" title="关闭交易信息"><i class="halflings-icon white icon-remove "></i></a>
													<a href="<%=path %>/blog/passBlog?id=${blogList.id}" class="btn mini blue icn-only" title="审核通过"><i class="halflings-icon white icon-ok "></i></a>
												</td>										
											</tr>
										</c:forEach>
									</tbody>
								...
								<div class="pull-right">
									...
									<ul>	
										...
										<c:if test="${myBlogList.pc > 1 }">						
											<li>
						            			  <a href="<%=path%>/blog/getManagerBlogList?pc=${blogList.pc-1}" aria-label="Previous">
										...
										<c:forEach var="i" begin="${begin }" end="${end }">
											<c:choose>
												...
												<c:otherwise>
												 <li><a href="<%=path%>/blog/getManagerBlogList?pc=${i}">${i }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>        
							            <c:if test="${blogList.pc < blogList.tp }">															
											<li>
					             			 <a href="<%=path%>/blog/getManagerBlogList?pc=${blogList.pc+1}" aria-label="Next">
	...
	<script src="<%=path%>/media/js/manager/blog/blogList.js" type="text/javascript"></script>  

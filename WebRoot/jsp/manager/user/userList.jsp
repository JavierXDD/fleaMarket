						...
						<div class="portlet box light-grey">
							...
							<div class="portlet-body">
								<table class="table table-striped table-bordered table-hover" id="userList">
									...
									<tbody>
										<c:forEach items="${userList.beanList }" var="user" varStatus="status" >										
											<tr class="odd gradeX">
												<td>${status.count} </td>
												<td>${user.nickname}</td>
												<td>${user.name}</td>
												...
												<c:if test="${user.isUse eq 0}">
													...
													<td>
														<a href="<%=path%>/user/toUserDetail?id=${user.id}" class="btn mini purple  icn-only" title="查看"><i class="halflings-icon white user "></i></a>
														
														<a href="<%=path %>/user/toUserEdit?id=${user.id}" class="btn mini blue icn-only" title="编辑"><i class="halflings-icon white pencil "></i></a>
														<a href="javascript:void(0);" onclick="forbidden('${user.id}','${user.nickname}')"  class="btn mini red icn-only" title="停用"><i class="halflings-icon white ban-circle"></i></a>																																										
												...
												<c:if test="${user.isUse eq 1}">
													...
													<td>
														...
														<a href="<%=path %>/user/toUserEdit?id=${user.id}" class="btn mini blue icn-only" title="编辑"><i class="halflings-icon white pencil "></i></a>
														<a href="javascript:void(0);" onclick="using('${user.id}','${user.nickname}')" class="btn mini red icn-only" title="启用"><i class="halflings-icon white ok-circle"></i></a>																											
								...
								<div class="pull-right">
									...
									<ul>	
										<%-- 如果总页数大于1页，那么《显示出来！ --%>									
										<c:if test="${userList.pc > 1 }">						
											<li>
						            			  <a href="<%=path%>/user/getUserList?pc=${userList.pc-1}" aria-label="Previous">
										...
										<c:choose>
											...
											<c:when test="${userList.tp <= 10 }">
												<c:set var="begin" value="1" />
												<c:set var="end" value="${userList.tp }" />
											</c:when>
											<c:otherwise>
												<%-- 当总页数>10时，通过公式计算出begin和end --%>
												<c:set var="begin" value="${userList.pc-5 }" />
												<c:set var="end" value="${userList.pc+4 }" />
												<%-- 头溢出 --%>
												<c:if test="${begin < 1 }">
													<c:set var="begin" value="1" />
												...
												<%-- 尾溢出 --%>
												<c:if test="${end > userList.tp }">
													<c:set var="begin" value="${userList.tp - 9 }" />
													<c:set var="end" value="${userList.tp }" />
												</c:if>
											</c:otherwise>
										</c:choose>
										...
										<c:forEach var="i" begin="${begin }" end="${end }">
											<c:choose>
												<c:when test="${i eq userList.pc }">
													 <li><a>${i }</a></li>
												</c:when>
												<c:otherwise>
												 <li><a href="<%=path%>/user/getUserList?pc=${i}">${i }</a></li>
												</c:otherwise>
											</c:choose>
										</c:forEach>        
							            <c:if test="${userList.pc < userList.tp }">															
											<li>
					             			 <a href="<%=path%>/user/getUserList?pc=${userList.pc+1}" aria-label="Next">
